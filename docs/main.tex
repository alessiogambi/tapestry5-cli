...
\paragraph{For the moment we do not build CLIParser with ChainBuilder}  because we need to capture all the steps into uniform commands, but each of them has different inputs/dependencies that we must consider properly.

\paragraph{Define validation constraints on inputs and in general combine multiple inputs to be validated together} This can be done by define two levels of validation, similarly to client and server side (that in tapestry can be done by raising a validation exception inside components once the Validate* events are bubbled!): The first level is provided to validate basic properties with JSR 303, and the second level (only triggered if the first was ok) that use the collected Validators provided by the user to combine inputs (identified by position!) and parsed options.
{\bf NOTE:} At the moment we provide only the scaffolding for creating a pipeline of validators, each stage consist of the application of a validator and a filter that accumulate validation constrains as the pipeline goes on.
