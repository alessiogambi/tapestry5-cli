...
\paragraph{Exporting Symbols or contributing a new symbol provider to
SymbolSource?} An alternative way to provide option values and such is to
provide a option Access service, similar to SymbolSource that we can inject and
query. This would make it easier to access simbols and may rely on annotations
to inject options. The downside is that we need to yet provide another service,
while we can simply rely on symbols which is a core tap service. 
...
\paragraph{@ParsingOption FIELD annotation} Beans contributed for validation can use the annotation to bind a specific field of the class with the relative parsing option. {\bf NOTE:} the bean MUST respect the bean Java convention, therefore getter/setter MUST be always specified for this to work.
